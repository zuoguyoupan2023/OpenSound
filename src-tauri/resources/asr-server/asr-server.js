...
async function engineReadiness(mf) {
  ...
  // 2026-09-08 Win 实测：vendored Matcha-TTS 缺 matcha/models 不在 pip 依赖、也不在引擎清单 runtime 里，
  // 但 cosyvoice.flow/decoder 在 hyperpyyaml 构造时会 import 它 → 缺了就 8003 秒退（No module named
  // 'matcha.models'）。就绪口径不覆盖这里会误报 ready，卡片只剩「重启服务」（重启必崩）没有「检测/修复」。
  if (mf.id === 'cosyvoice-clone') {
    const matchaModels = path.join(__dirname, 'vendor', 'cosyvoice', 'third_party', 'Matcha-TTS', 'matcha', 'models');
    if (!existsSync(matchaModels)) {
  ...
  'cosyvoice-clone': async (ctx, opts = {}) => {
    ...
    if (!existsSync(path.join(matchaV, 'models'))) {
      ...
      try {
        ...
        // ⚠️ jsdelivr flat 清单的 name 带前导 '/'（如 /matcha/models/components/decoder.py）——
        // 2026-09-09 Win 实测：不剥掉前导斜杠直接 startsWith('matcha/models/') 恒空 → 补拉永远中止。
        files = (tree.files || []).map((f) => String(f.name || '').replace(/^\/+/, '')).filter((n) => n.startsWith('matcha/models/'));
      } catch (e) {
